...
### 4. Веб-интерфейс (гибридный поиск)

Веб-страница hybrid-поиска раздаётся на корне `/` (страница сравнения FTS /
Vector / Hybrid была убрана с корня; её HTML `eval_page.html` не раздаётся —
внутренний `/api/compare` при этом сохранён для диагностики выдачи). Клик по
...
Публичный доступ по алиасу `https://kb.tradesoft.corp/` идёт через nginx
reverse-proxy (см. TECHNICAL.md, раздел 11.4).
