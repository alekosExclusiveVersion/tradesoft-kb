...
- **Самоподписанный сертификат** — `/opt/homebrew/etc/nginx/ssl/kb.tradesoft.corp.{crt,key}`,
  SAN: `DNS:kb.tradesoft.corp`, `IP:192.168.128.56`, `IP:127.0.0.1` (действует 825 дней).
- **Запись в `/etc/hosts`** — `127.0.0.1 kb.tradesoft.corp` (для доступа с этой машины;
  коллеги в корп. сети ts-work добавляют `192.168.128.56 kb.tradesoft.corp` в свой hosts
  или используют IP `http://192.168.128.56:8055/`).

Точки входа:
- `https://kb.tradesoft.corp/` — гибридный поиск (основной доступ)
- `http://192.168.128.56:8055/` — прямой доступ без TLS (внутри корп. сети)

**Доверие сертификату (открытие без предупреждения).** Сертификат
самоподписанный, поэтому для браузеров без предупреждения его нужно добавить
в доверенные корневые ЦС macOS (System Keychain) — как сделано для
`b24.search`:

```bash
sudo security add-trusted-cert -d -r trustRoot \
  -k "/Library/Keychains/System.keychain" \
  /opt/homebrew/etc/nginx/ssl/kb.tradesoft.corp.crt
```

После добавления нужно **полностью перезапустить браузер** (⌘Q), иначе он
продолжит показывать «соединение не защищено» из-за кэша по данному хосту.
Проверка доверия:

```bash
security verify-cert -c /opt/homebrew/etc/nginx/ssl/kb.tradesoft.corp.crt
# → ...certificate verification successful.
```

Для коллег из подсети: выполнить ту же команду на их машинах (путь к `.crt`
можно передать отдельно), либо добавить сертификат в корпоративный MDM/CA.
